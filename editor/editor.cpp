...
static bool Splitter(bool split_vertically, float thickness, float *size1,
...
struct Editor : public Application {
  ...
  Node *SpawnInputActionNode() {
    m_Nodes.emplace_back(GetNextId(), "InputAction Fire",
                         ImColor(255, 128, 128));
  ...
  Node *SpawnTraceByChannelNode() {
    ...
    m_Nodes.back().Inputs.emplace_back(GetNextId(), "Draw Debug Type",
                                       PinType::Bool);
    m_Nodes.back().Inputs.emplace_back(GetNextId(), "Ignore Self",
                                       PinType::Bool);
    m_Nodes.back().Outputs.emplace_back(GetNextId(), "", PinType::Flow);
  ...
  void OnStart() override {
    ...
    config.LoadNodeSettings = [](ed::NodeId nodeId, char *data,
  ...
  void ShowStyleEditor(bool *show = nullptr) {
    ...
    ImGui::DragFloat("Selected Node Border Offset",
                     &editorStyle.SelectedNodeBorderOffset, 0.1f, -40.0f,
                     40.0f);
  ...
  void ShowLeftPane(float paneWidth) {
    ...
    for (auto &node : m_Nodes) {
      ...
      auto iconPanelPos =
          start + ImVec2(paneWidth - ImGui::GetStyle().FramePadding.x -
...
int Main(int argc, char **argv) {
  Editor ed("Blueprints", argc, argv);
